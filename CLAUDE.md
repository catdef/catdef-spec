...
- **Remove Thingalog-specific references from the catdef repo.** Thingalog was catdef's first runtime; residual references to it remain in the catdef repo and should be removed, since per value #2 catdef is not bound to any implementation. Two categories: (1) the `.thingalog` file extension (not one of the three canonical catdef extensions — `.openthing` / `.opencatalog` / `.catdef` — defined at `CATDEF_SPEC.md` "File Extensions"), and (2) Thingalog-domain example URLs. Locations: (a) fixture filenames in `conformance/fixtures/` (10 files) and the test-code call sites that open them in `conformance/test_parsing.py`, `test_field_values.py`, `test_photo_transforms.py`; (b) file-extension prose in `conformance/README.md` lines 9, 28, 36, 38–43; (c) example URLs at `CATDEF_SPEC.md:243` (`builder.thingalog.com`) and `CATDEF_SPEC.md:1096` (`scotts-tools.thingalog.app`), both to be replaced with generic example domains; (d) `samples/watches.thingalog` referenced by `conformance/test_parsing.py:94`. Extension rename target is not uniform — fixtures with schema + data become `.opencatalog`, schema-only cases become `.catdef`, single-object cases become `.openthing` — so (a) requires per-file classification followed by coordinated fixture rename + test-code update. Patch-level when decoupled by category.
- **Stale field-type count in `conformance/README.md`.** Line 11 reads `test_fields.py     field type rendering (all 11 types)`; the spec declares 13 field types at `CATDEF_SPEC.md` "Field Types". Preferred fix is to say "all field types" rather than pinning to any number, so the spec remains the source of truth. Patch-level.
- **Conformance suite scope note missing from `conformance/README.md`.** The repo-level README advertises the suite as "98 tests"; `conformance/README.md` itself gives no indication of scope. Add a one-line note ("The suite contains 98 tests organized by subject area" or similar) so an adopter landing on the conformance README can calibrate expectations. Patch-level.
