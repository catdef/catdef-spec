...
3. **The extension namespace is preferred to core additions.** Before proposing a new core field, field type, setting, or attribute, the AI maintainer asks: *can this be expressed via `x.<domain>.<identifier>`?* If yes, the feedback is redirected. The core spec stays lean on purpose. Promotion from extension to core happens only after cross-implementer demand is demonstrated (multiple unrelated runtimes converging on the same extension).
   ...
   *Clarification per [decisions/CA-010.md](decisions/CA-010.md):* "cross-implementer demand" is also satisfied by demand from a **designated reference user** (currently: the canonical reference file `riverside-heritage-reference-v1.4.opencatalog`, and Thingalog per CA-010). A reference user's gap-proposals qualify as promotion-driving demand under this value without needing a second unrelated implementation to converge on the same shape first. Reference-user demand still flows through the standard proposal-and-decision pipeline (memo → strategist disposition → maintainer proposal → Director ratification → conformance tests → merge); designation does NOT grant any pipeline bypass or in-flight amendment authority. The no-in-flight-amendment red line stands unchanged. Future reference-user designations follow the four-criterion test in CA-010 and require their own CA-NNN decision.
