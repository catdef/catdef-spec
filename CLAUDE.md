...
- **Footer version stamp on CATDEF_SPEC.md.** Reads "v1.1" in the footer; body is v1.3. Patch-level fix.
- **Runtime-specific branding in conformance/README.md.** Contains references to a specific runtime name where "catdef" is the correct term. Per the governance principle that no implementation owns the spec, the conformance suite's own documentation should use the standard's name. Patch-level fix.
- **L1 field type support wording.** The field types section says runtimes SHOULD support Date, Money, Boolean, etc.; the L1 conformance definition says L1 only supports String/Integer/RichText/Enumerated/Photo. An L1-only renderer is therefore simultaneously conformant-at-L1 and not-meeting-SHOULD. Clarify whether SHOULD applies only at L2+ or whether L1's list should expand.
