...
| `photos/` | 22 real photos sourced from Pexels and Unsplash, referenced by filename from the JSON. |
...
- **Stanley Rule & Level Co.** is a real historical manufacturer (founded 1857, New Britain, Connecticut; acquired by Stanley Works in 1920). It is referenced in its historical form to ground the canonical in a recognizable example and echo [CATDEF_SPEC.md's Stanley subcat example](../CATDEF_SPEC.md). The Bailey-pattern plane (item 3) is described consistent with the historical record. The Maker subcat's `Logo` Photo field is deliberately left empty for this entry — using a generic stock image to stand in for a real company's trademark would misrepresent the historical record.
